...
Twittergs.Client=class{
  constructor({property=PropertiesService.getUserProperties(),CLIENT_ID=property.getProperty("CLIENT_ID"),CLIENT_SECRET=property.getProperty("CLIENT_SECRET"),API_KEY=property.getProperty("API_KEY"),API_SECRET=property.getProperty("API_SECRET"),name,oauthVersion,ACCESS_TOKEN,ACCESS_TOKEN_SECRET,restTime=1000}={}){
    ...
    this.property=new Twittergs.Property(property,this)
    if(this.oauthVersion==="2.0"){
      ...
      this.BASIC=Utilities.base64Encode(Twittergs.Util.parcentEncode(this.clientId)+":"+Twittergs.Util.parcentEncode(this.clientSecret))
   ...
   */
  authorize(scopes=Twittergs.TWITTER_API_DATA.scopes){
    if(this.oauthVersion==="2.0"){
      const code_verifier=Twittergs.Util.makeNonce(32)
      ...
      return `https://twitter.com/i/oauth2/authorize?response_type=code&client_id=${this.clientId}&redirect_uri=${Twittergs.Util.parcentEncode(Twittergs.Util.getCallBackURL())}&scope=${scopes.join("+")}&state=${state}&code_challenge=${challenge}&code_challenge_method=S256`
    }else{
      const state=ScriptApp.newStateToken()
        .withTimeout(3600)
        .withArgument("name",this.name)
      ...
      const {oauth_token,oauth_token_secret}=this.fetch("https://api.twitter.com/oauth/request_token",{
        ...
        payload:{
          oauth_callback:Twittergs.Util.getCallBackURL()+"?state="+state
        }
      })
  ...
  _makeSignature({method,url,oauthParams}={}){
    ...
    const paramsResult=params.map(([key,value])=>`${Twittergs.Util.parcentEncode(key)}=${Twittergs.Util.parcentEncode(value)}`).join("&")
    const base=`${method}&${Twittergs.Util.parcentEncode(url)}&${Twittergs.Util.parcentEncode(paramsResult)}`
    const signing=`${Twittergs.Util.parcentEncode(this.apiSecret)}&${Twittergs.Util.parcentEncode(this.oauthTokenSecret||"")}`
  ...
  isAuthorized(e){
    ...
    if(this.oauthVersion==="2.0"){
      ...
      const { refresh_token, access_token,scope} = JSON.parse(UrlFetchApp.fetch("https://api.twitter.com/2/oauth2/token",{
        ...
        payload:{
          ...
          code,
          code_verifier,
          redirect_uri:Twittergs.Util.getCallBackURL()
        }
      }))
      this.property.setProperties({
        refresh_token,
    ...
    let response=Twittergs.Util.parseParam(UrlFetchApp.fetch("https://api.twitter.com/oauth/access_token",{
      method:"POST",
      payload:{
    ...
    if(this.name==="@auto"){
      ...
      this.property=new Twittergs.Property(this.property.property,this)
    }
  ...
  static refreshAll({CLIENT_ID,CLIENT_SECRET,names}={}){
    names.forEach((name)=>{
      new Twittergs.Client({CLIENT_ID,CLIENT_SECRET,name}).refresh()
    })
  }
  /**
   * 適切な認証情報を載せてfetchします
  ...
  fetch(url,options){
    ...
    if(this.oauthVersion==="2.0"){
      ...
      if ((options.method === "GET") && options.queryParameters) {
        let uriOption=Twittergs.Util.buildParam(options.queryParameters)
      ...
      const oauthOptions={
        ...
        oauth_consumer_key:this.apiKey,
        oauth_nonce:Twittergs.Util.makeNonce(),
        oauth_signature_method:"HMAC-SHA1",
        oauth_timestamp:Math.floor(Date.now()/1000)+"",
        oauth_version:"1.0"
      }
      if(options.contentType==="application/x-www-form-urlencoded"&&options.payload){
        url+=(url.includes("?")?"&":"?")+Twittergs.Util.buildParam(options.payload)
      ...
      const authorizationString="OAuth "+Object.keys(oauthOptions).sort().map(key=>
        `${Twittergs.Util.parcentEncode(key)}="${Twittergs.Util.parcentEncode(oauthOptions[key])}"`
      ).join(", ")
      ...
      switch(response.getHeaders()["Content-Type"].split(";")[0]){
        ...
        case "text/html":return Twittergs.Util.parseParam(response.getContentText());break
  ...
  searchTweets(queryParameters){
    ...
    return Twittergs.Util.shapeData(response,v=>new Twittergs.Tweet(v,this),"statuses")
  }
  /**
   * idで指定したツイートを取得します
  ...
  postTweet(payload){
    ...
    return new Twittergs.ClientTweet(response.data, this)
  }
  ...
  getListById(id,queryParameters){
    ...
    return new Twittergs.List(response.data,this)
  }
  /**
   * ユーザーを検索します
  ...
  searchUsers(queryParameters){
    ...
    return response.map(v=>new Twittergs.User(v,this))
  }
  ...
  static getAuthorizedUsers(property=PropertiesService.getUserProperties()){
    ...
    return [
      data.filter(v=>v.version==="1.0a").map(v=>v.n).filter(name=>new Twittergs.Client({name,oauthVersion:"1.0a"}).hasAuthorized()),
      data.filter(v=>v.verison==="2.0").map(v=>v.n).filter(name=>new Twittergs.Client({name,oauthVersion:"1.0a"}).hasAuthorized())
    ]
  }
}
...
Twittergs.AppOnlyClient=class{
  ...
  fetch(url,options){
    ...
    if(options.queryParameters){
      url+=(url.includes("?")?"":"?")+Twittergs.Util.buildParam(options.queryParameters)
      delete options.queryParameters
    }
    return JSON.parse(UrlFetchApp.fetch(url,options))
  }
...
Twittergs.List=class{
  ...
  getMembers(queryParameters){
    ...
    return Twittergs.Util.shapeData(response,new Twittergs.User(v,this.client))
  }
}
Twittergs.Tweet=class{
  constructor(d,client){
    if(typeof d==="string")this.id=d
    else Object.assign(this,d)
...
Twittergs.ClientUser=class extends Twittergs.User{
  /**
   * ブロックしているユーザーを取得します
